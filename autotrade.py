...
access = "15WMUrgRkZGjpGWojN1e3JUhF9LpLtXASIxdOtpM"
secret = "RI1LIlra2ZjammIuK5IShyATinxCVAJ08TIWTDB2"
...
# now가 start time과 'end time(start time에 하루를 더한 시간)에서 10초를 뺀 시간' 사이에 있고 
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-SOL", 0.3)
            current_price = get_current_price("KRW-SOL")
            #current price가 target price보다 높고
